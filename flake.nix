{
  ...
  outputs = { self, nixpkgs, nixpkgs-wireshark, libosi-src, nix-filter }:
    let
      ...
      pkgsWithConfig = {
        config-file ? ./panda/plugins/config.panda,
        target-list ? [
          "x86_64-softmmu"
          "i386-softmmu"
          "arm-softmmu"
        ...
        config = builtins.readFile config-file;
        ...
        panda = pkgs.stdenv.mkDerivation {
          ...
          postPatch = ''
            ...
            substituteInPlace panda/plugins/pri_dwarf/*.{h,cpp} \
              --replace '<libdwarf/' '<'
            cp "${config-file}" panda/plugins/config.panda
          ...
          configureFlags = [
            "--target-list=${builtins.concatStringsSep "," target-list}"
            "--disable-numa"
            # TODO: "--enable-llvm"
          ];
          postInstall = ''
